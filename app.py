...
# ==========================================
# 🎨 Premium UI/UX: Animations & Glow Effect
# ==========================================
st.markdown("""
<style>
    .stApp {
...
# ------------------------------------------------------------------
# Page 1: Student Risk Analysis (Threshold Fixed)
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
    ...
    if submit_app:
        try:
            ...
            # THE FIX: Force Float64 as DataFrame to satisfy both Pipeline and XGBoost
            df_app = pd.DataFrame([input_dict])
            for col in df_app.columns:
                df_app[col] = pd.to_numeric(df_app[col]).astype(float)

            if hasattr(app_model, 'feature_names_in_'):
                df_app = df_app[list(app_model.feature_names_in_)]
