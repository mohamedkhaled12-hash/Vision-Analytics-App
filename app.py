...
<style>
    ...
    .gradient-text {
        ...
        font-weight: 900; font-size: 3.2rem; text-align: center; text-shadow: 0px 4px 20px rgba(168, 85, 247, 0.3);
    }
    [data-testid="stForm"], .metric-card {
        background: rgba(15, 23, 42, 0.45) !important; backdrop-filter: blur(20px);
        border-radius: 20px; padding: 30px; border: 1px solid rgba(255, 255, 255, 0.08);
    }
    [data-testid="baseButton-secondary"] {
        background: linear-gradient(135deg, #06B6D4 0%, #3B82F6 100%) !important;
        border: none !important; padding: 16px 24px !important; border-radius: 12px !important; font-weight: 800 !important;
    }
</style>
...
# ------------------------------------------------------------------
# Page 2: App Behavior Analysis (النسخة المُدرعة - Bulletproof)
# ------------------------------------------------------------------
else:
    st.markdown("<h3>📱 App Behavior Tech-Metrics</h3>", unsafe_allow_html=True)
    ...
    if submit_app:
        try:
            # 1. التخلص من الكلمات تماماً وتحويلها لرقم فوراً
            gender_numeric = 1.0 if gender == "Male" else 0.0
            
            # 2. بناء قاموس نقي 100% من الأرقام (مستحيل يكون فيه Object)
            pure_data = {
                ...
                'Age': float(age),
                'Gender': float(gender_numeric)
            }
            
            # 3. إنشاء DataFrame أولي
            df_app = pd.DataFrame([pure_data])
            
            # 4. إعادة بناء DataFrame مطابق للموديل بدقة
            if hasattr(app_model, 'feature_names_in_'):
                expected_cols = list(app_model.feature_names_in_)
                
                # إنشاء جدول نظيف جداً وفارغ
                df_clean = pd.DataFrame(columns=expected_cols)
                df_clean.loc[0] = 0.0  # ملء بصفر كأرقام عشرية
                
                # مطابقة الأعمدة بذكاء
                for ex_col in expected_cols:
                    for my_col in df_app.columns:
                        if my_col.strip().lower() == ex_col.strip().lower():
                            df_clean.at[0, ex_col] = df_app.at[0, my_col]
                
                # إرسال الجدول النظيف للموديل
